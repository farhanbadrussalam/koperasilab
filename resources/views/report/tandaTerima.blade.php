...
@section('content')
            ...
            if ($getPertanyaan[$i]->type == 1) {
                $tdContent .=
                    ...
                    '.</td>
                                <td>
                                    ' .
                    $getPertanyaan[$i]->pertanyaan .
                    ' :<br>
                                    <span class="text-secondary">' .
                    $data->tandaterima[$i]->jawaban .
                    '</span>
                                </td>
                            ';
            } else {
            ...
            if ($getPertanyaan[$i + $half]->type == 1) {
                $tdContent .=
                    ...
                    '.</td>
                                <td>
                                    ' .
                    $getPertanyaan[$i + $half]->pertanyaan .
                    ' :<br>
                                    <span class="text-secondary">' .
                    $data->tandaterima[$i + $half]->jawaban .
                    '</span>
                                </td>
                            ';
            } else {
    ...
    <table class="table-footer">
        <tr>
            <td colspan="2">Jumlah : <span
                    class="text-secondary">{{ $data->jumlah_pengguna . ' Pengguna +' . $data->jumlah_kontrol . ' Kontrol' }}</span>
                <span>{{ $data->periode > 0 ? 'Periode ' . $data->periode : 'Periode zero Check' }}</span></td>
        </tr>
        <tr>
            <td width="50%">Tanggal Penerimaan : <span
                    class="text-secondary">{{ convert_date($data->dokumen[0]->created_at, 2) }}</span></td>
            <td>Tanggal Selesai Pengujian : <span
                    class="text-secondary">{{ $selesaiPengujian ? convert_date($selesaiPengujian, 2) : '.......' }}</span>
            </td>
        </tr>
        <tr>
            <td>&nbsp;</td>
        ...
        <tr>
            <td width="50%">
                <div class="text-center d-flex">
                    ...
                    <img class="ttd-image" src="{{ $data->pelanggan->ttd ? $data->pelanggan->ttd : $ttd_default }}"
                        alt="ttd" srcset="ttd">
                    <div class="flex-1">(
                        {{ $data->pelanggan ? $data->pelanggan->name : '................................' }} )</div>
                    <div>Nama jelas</div>
                </div>
            </td>
            <td width="50%">
                <div class="text-center d-flex">
                    ...
                    <img class="ttd-image" src="{{ $data->ttd ? $data->ttd : $ttd_default }}" alt="ttd"
                        srcset="ttd">
                    <div class="flex-1">(
                        {{ $data->signature ? $data->signature->name : '................................' }} )</div>
                    <div>Nama jelas</div>
                </div>
            </td>
        </tr>
