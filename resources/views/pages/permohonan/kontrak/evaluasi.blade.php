        ...
        <div class="container-fluid">
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-8">
                        ...
                        <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
                            <div class="mb-3">
                                ...
                                <div class="input-group shadow-sm rounded-3 overflow-hidden">
                                    ...
                                    <select name="selectAlamat" id="selectAlamat"
                                        class="form-select border-start-0 py-3 cursor-pointer">
                            ...
                            <div class="mb-0">
                                ...
                                <textarea name="txt_alamat" id="txt_alamat" cols="30" rows="3"
                                    class="form-control bg-light border-0 rounded-3 p-3" readonly
                                    placeholder="Detail alamat akan otomatis terisi setelah memilih jenis alamat..."></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Tombol Aksi Bawah -->
                    <div
                        class="card border-0 shadow-sm rounded-4 p-4 text-end bg-white gap-2 d-flex justify-content-between flex-row">
                        <a href="{{ url()->previous() }}" class="btn btn-light rounded-pill px-4 me-2 text-muted">Batal</a>
                        <button class="btn btn-primary rounded-pill px-5 flex-grow-1 fw-bold shadow-sm"
                            onclick="buatPermohonan(this)">
                        ...
                        <div class="card border-0 shadow-sm rounded-4 mb-4 bg-white">
                            ...
                            <div class="card-body p-4">
                                @if ($periodeNow->start_date)
                                    <div class="mb-3">
                                        <label class="text-uppercase text-muted small fw-bold tracking-wide">Periode
                                            Pemakaian</label>
                                        <div
                                            class="p-3 bg-light rounded-3 border border-secondary border-opacity-25 mt-1 d-flex align-items-center">
                                            <i class="bi bi-clock-history text-secondary me-3 fs-4"></i>
                                            <div>
                                                <div class="fw-bold text-dark">Periode Aktif</div>
                                                <div class="small text-muted">{{ convert_date($periodeNow->start_date, 2) }}
                                                    - {{ convert_date($periodeNow->end_date, 2) }}</div>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if ($periode2Next)
                                    <div class="mb-3">
                                        <label class="text-uppercase text-muted small fw-bold tracking-wide">Periode
                                            Berikutnya</label>
                                        <div
                                            class="p-3 bg-primary bg-opacity-10 rounded-3 border border-primary border-opacity-25 mt-1 d-flex align-items-center">
                                            <i class="bi bi-arrow-right-circle text-primary me-3 fs-4"></i>
                                            <div>
                                                <div class="fw-bold text-primary">Periode Mendatang</div>
                                                <div class="small text-muted">
                                                    {{ $periode2Next ? convert_date($periode2Next->start_date, 2) : '-' }} -
                                                    {{ $periode2Next ? convert_date($periode2Next->end_date, 2) : '-' }}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                @if (!$periode2Next && !$isSewa)
                                    <div class="mb-3">
                                        <label class="text-uppercase text-muted small fw-bold tracking-wide">Pengembalian
                                            Ke-{{ $periodeNow->count_tld }}</label>
                                        <div
                                            class="p-3 bg-warning bg-opacity-10 rounded-3 border border-warning border-opacity-25 mt-1 d-flex align-items-center">
                                            <i class="bi bi-arrow-return-left text-warning me-3 fs-4"></i>
                                            <div>
                                                <div class="fw-bold text-warning">Jadwal Pengembalian</div>
                                                <div class="small text-muted">{{ convert_date($pengembalianStart, 2) }} -
                                                    {{ convert_date($pengembalianEnd, 2) }}</div>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <div class="card border-0 shadow-sm rounded-4 bg-white">
                            ...
                            <div class="card-body p-4">
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="p-3 bg-light rounded-3 border mt-1">
                                        <div class="fw-bold text-dark">
                                            {{ $kontrak->pelanggan->perusahaan->nama_perusahaan }}</div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="p-3 bg-light rounded-3 border mt-1">
                                        <span
                                            class="badge bg-secondary-subtle fw-normal rounded-pill text-secondary-emphasis fs-7 px-3">
                                ...
                                <div class="mb-0">
                                    ...
                                    <div class="p-3 bg-light rounded-3 border mt-1">
                                        <span
                                            class="fw-bold text-success">{{ formatCurrency($kontrak->total_harga) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
