                ...
                <div class="card mb-2 smooth-height">
                    <div class="card-body row align-items-center py-2">
                        <div class="col-auto">
                            ...
                            <div class="fs-5 my-2"><span class="fw-bold">${data.jenis_tld.name} - ${data.pelanggan.perusahaan.nama_perusahaan}</span> <span class="text-body-tertiary">#${data.no_kontrak}</span></div>
                            <div class="d-flex gap-3 text-body-tertiary">
                                ...
                                <div>${statusFormat('invoice', data.invoice.status)}</div>
                            </div>
                        </div>
                        <div class="col-auto ms-auto align-self-end d-flex gap-1">
                        ...
                        <div class="p-3" id="listPeriode${i}" style="display:none">
                            <div class="border-top py-2 d-flex justify-content-between align-items-center">
                                <div class="px-2">
                                    <span class="fw-semibold fs-6">Periode 1</span>
                                    <small class="text-body-tertiary"> - (1 April 2025 - 1 Juni 2025)</small>
                                    <small><span class="text-success ms-2"><i class="bi bi-check-circle-fill"></i> Terkirim</span></small>
                                </div>
                                <div class="d-flex align-items-center gap-3 text-secondary">
                                    <button class="btn btn-sm btn-outline-warning" onclick=""><i class="bi bi-file-earmark-text"></i> Evaluasi</button>
                                </div>
                            </div>
                            <div class="border-top py-2 d-flex justify-content-between align-items-center">
                                <div class="px-2">
                                    <span class="fw-semibold fs-6">Periode 2</span>
                                    <small class="text-body-tertiary"> - (1 Juli 2025 - 1 September 2025)</small>
                                    <small><span class="text-secondary ms-2"><i class="bi bi-dash-circle"></i> Belum dikirim</span></small>
                                </div>
                                <div class="d-flex align-items-center gap-3 text-secondary">
                                    <button class="btn btn-sm btn-outline-warning" onclick=""><i class="bi bi-file-earmark-text"></i> Evaluasi</button>
                                </div>
    ...
    for (const [i, data] of arrPeriode.entries()) {
        ...
        for (const doc of arrFind) {
            ...
            htmlDoc += `
                <div>
                    ...
                    <small>${statusFormat('pengiriman', findPeriode?.status)}</small>
                </div>
            `;
        ...
        if(data.permohonan){
            ...
            <div class="d-flex flex-column justify-content-center align-items-center">
                <div>
                    ${data.permohonan.jenis_layanan_parent.name}
                </div>
                <div>
                    ${statusFormat('permohonan', data.permohonan.status)} 
                </div>
            </div>
            `;
            isComplete ? evaluasiActive = true : evaluasiActive = false;
        }else{
            evaluasiActive ? htmlAction = `<a class="btn btn-sm btn-outline-primary" href="${base_url}/permohonan/kontrak/e/${dataKontrak[index].kontrak_hash}/${data.periode_hash}"><i class="bi bi-file-earmark-text"></i> Evaluasi</a>` : false;
